...
{ 
	...
	OV2640_PWDN(GPIO_PIN_RESET);	//POWER ON
	HAL_Delay(10);
	OV2640_RST(GPIO_PIN_RESET);	//复位OV2640
	HAL_Delay(10);
	OV2640_RST(GPIO_PIN_SET);	//结束复位 
	
  SCCB_Init();        		//初始化SCCB 的IO口	 
	while(SCCB_WR_Reg(OV2640_DSP_RA_DLMT, 0x01));	//操作sensor寄存器
 	while(SCCB_WR_Reg(OV2640_SENSOR_COM7, 0x80));	//软复位OV2640
	HAL_Delay(50); 
